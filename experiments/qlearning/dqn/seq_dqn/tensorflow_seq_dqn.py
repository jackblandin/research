...
Objectives
----------
1   Learn optimal policy to Tiger Env using a sequence of inputs to capture
    the uncertainty in observations.
2   Improve TensorFlow skills by deviating from the given DQN algorithm from
    the Udemy course.
...
class TensorFlowSeqDQN:
    """
    A Deep Q Network. Uses TensorFlow. Uses a sequence of observations as input
    to the Q network.
    ...
    def predict(self, o, update=False):
        """
        ...
        ----------
        o : array-like
            SINGLE observation. Sequence will be fetched and appended.
        update : bool, default False
            If True, last_n_obs attribute will be updated with `o` observation.
    ...
    def train(self, target_network, n):
        ...
        # replay buffer.
        ##
        action_values = target_network.predict(flat_next_states)
    ...
    def sample_action(self, o, eps):
        ...
        if np.random.random() < eps:
            ...
            return np.argmax(self.predict(o, update=True))
